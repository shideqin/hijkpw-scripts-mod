#!/usr/bin/env bash

# ============================================================
# Xray VLESS + REALITY + XTLS Vision 一键安装脚本
#
# Protocol : VLESS
# Transport: TCP
# Security : REALITY
# Flow     : xtls-rprx-vision
#
# Compatible target:
#   Xray-core current release
#   v2rayN / v2rayNG using current Xray core
#
# Config:
#   /usr/local/etc/xray/config.json
#
# Backup:
#   /usr/local/etc/xray/backup/
# ============================================================

set -e

XRAY_BIN="/usr/local/bin/xray"
XRAY_DIR="/usr/local/etc/xray"
XRAY_CONFIG="${XRAY_DIR}/config.json"
BACKUP_DIR="${XRAY_DIR}/backup"

INSTALL_SCRIPT_URL="https://github.com/XTLS/Xray-install/raw/main/install-release.sh"

RED='\033[0;31m'
GREEN='\033[0;32m'
YELLOW='\033[1;33m'
BLUE='\033[0;34m'
NC='\033[0m'

info() {
    echo -e "${BLUE}[INFO]${NC} $1"
}

success() {
    echo -e "${GREEN}[OK]${NC} $1"
}

warn() {
    echo -e "${YELLOW}[WARN]${NC} $1"
}

error() {
    echo -e "${RED}[ERROR]${NC} $1"
}

die() {
    error "$1"
    exit 1
}

pause_enter() {
    echo
    read -rp "按 Enter 继续..."
}

check_root() {
    if [ "$(id -u)" != "0" ]; then
        die "请使用 root 用户运行此脚本"
    fi
}

check_os() {

    if [ ! -f /etc/os-release ]; then
        die "无法识别操作系统"
    fi

    . /etc/os-release

    info "操作系统: ${PRETTY_NAME}"

    case "${ID}" in
        debian|ubuntu|centos|rocky|almalinux|fedora)
            ;;
        *)
            warn "当前系统 ${ID} 未经过此脚本测试"
            read -rp "是否继续？[y/N]: " answer

            if [[ ! "$answer" =~ ^[Yy]$ ]]; then
                exit 0
            fi
            ;;
    esac
}

check_commands() {

    local missing=""

    for cmd in curl openssl sed grep awk; do
        if ! command -v "$cmd" >/dev/null 2>&1; then
            missing="${missing} ${cmd}"
        fi
    done

    if [ -n "$missing" ]; then
        warn "缺少命令:${missing}"

        if command -v apt-get >/dev/null 2>&1; then

            apt-get update

            apt-get install -y \
                curl \
                openssl \
                ca-certificates \
                sed \
                grep \
                gawk

        elif command -v dnf >/dev/null 2>&1; then

            dnf install -y \
                curl \
                openssl \
                ca-certificates \
                sed \
                grep \
                gawk

        elif command -v yum >/dev/null 2>&1; then

            yum install -y \
                curl \
                openssl \
                ca-certificates \
                sed \
                grep \
                gawk

        else
            die "无法自动安装依赖"
        fi
    fi
}

backup_config() {

    if [ -f "$XRAY_CONFIG" ]; then

        mkdir -p "$BACKUP_DIR"

        local backup_file

        backup_file="${BACKUP_DIR}/config-$(date +%Y%m%d-%H%M%S).json"

        cp "$XRAY_CONFIG" "$backup_file"

        success "旧配置已备份：$backup_file"
    fi
}

install_xray() {

    info "检查 Xray..."

    if [ -x "$XRAY_BIN" ]; then

        success "检测到 Xray：$($XRAY_BIN version 2>/dev/null | head -n 1 || true)"

        read -rp "是否更新/重新安装 Xray？[Y/n]: " answer

        if [[ "$answer" =~ ^[Nn]$ ]]; then
            return
        fi
    else
        info "未检测到 Xray，开始安装..."
    fi

    bash -c "$(curl -L "$INSTALL_SCRIPT_URL")" @ install

    if [ ! -x "$XRAY_BIN" ]; then
        die "Xray 安装失败"
    fi

    success "Xray 安装完成"

    "$XRAY_BIN" version || true
}

generate_uuid() {

    "$XRAY_BIN" uuid
}

generate_reality_keys() {

    local result

    result="$("$XRAY_BIN" x25519)"

    REALITY_PRIVATE_KEY="$(echo "$result" | awk -F': ' '/PrivateKey:/ {print $2}')"
    REALITY_PUBLIC_KEY="$(echo "$result" | awk -F': ' '/Password:/ {print $2}')"

    # 某些新版输出格式可能不同
    if [ -z "$REALITY_PRIVATE_KEY" ]; then
        REALITY_PRIVATE_KEY="$(echo "$result" | awk -F': ' '/Private key:/ {print $2}')"
    fi

    if [ -z "$REALITY_PUBLIC_KEY" ]; then
        REALITY_PUBLIC_KEY="$(echo "$result" | awk -F': ' '/Public key:/ {print $2}')"
    fi

    if [ -z "$REALITY_PRIVATE_KEY" ] || [ -z "$REALITY_PUBLIC_KEY" ]; then
        echo
        echo "$result"
        echo

        die "无法自动解析 Reality 密钥"
    fi
}

generate_short_id() {

    # 16 位十六进制
    SHORT_ID="$(openssl rand -hex 8)"
}

get_server_ip() {

    SERVER_IP=""

    # 优先 IPv4
    SERVER_IP="$(curl -4 -s --max-time 5 https://api.ipify.org || true)"

    if [ -z "$SERVER_IP" ]; then
        SERVER_IP="$(curl -4 -s --max-time 5 https://ifconfig.me || true)"
    fi

    if [ -z "$SERVER_IP" ]; then
        SERVER_IP="$(hostname -I 2>/dev/null | awk '{print $1}' || true)"
    fi

    if [ -z "$SERVER_IP" ]; then
        warn "无法自动获取服务器公网 IP"
        read -rp "请输入服务器 IP/域名: " SERVER_IP
    fi
}

input_port() {

    echo
    read -rp "VLESS 端口 [443]: " XRAY_PORT

    XRAY_PORT="${XRAY_PORT:-443}"

    if ! [[ "$XRAY_PORT" =~ ^[0-9]+$ ]]; then
        die "端口必须是数字"
    fi

    if [ "$XRAY_PORT" -lt 1 ] || [ "$XRAY_PORT" -gt 65535 ]; then
        die "端口范围必须是 1-65535"
    fi
}

input_reality_dest() {

    echo
    echo "============================================================"
    echo "Reality 目标站点"
    echo "============================================================"
    echo
    echo "例如："
    echo "  www.cloudflare.com:443"
    echo "  www.microsoft.com:443"
    echo "  www.yahoo.com:443"
    echo
    echo "要求目标站点支持 TLS 1.3。"
    echo

    read -rp "Reality 目标站点 [www.cloudflare.com:443]: " REALITY_DEST

    REALITY_DEST="${REALITY_DEST:-www.cloudflare.com:443}"

    REALITY_SERVER_NAME="${REALITY_DEST%%:*}"

    if [ -z "$REALITY_SERVER_NAME" ]; then
        die "无法解析 Reality serverName"
    fi
}

input_server_name() {

    echo
    read -rp "Reality SNI [${REALITY_SERVER_NAME}]: " REALITY_SNI

    REALITY_SNI="${REALITY_SNI:-$REALITY_SERVER_NAME}"
}

input_fingerprint() {

    read -rp "客户端指纹 [chrome]: " FINGERPRINT

    FINGERPRINT="${FINGERPRINT:-chrome}"
}

create_config() {

    mkdir -p "$XRAY_DIR"

    backup_config

    cat > "$XRAY_CONFIG" <<EOF
{
  "log": {
    "loglevel": "warning"
  },
  "inbounds": [
    {
      "listen": "0.0.0.0",
      "port": ${XRAY_PORT},
      "protocol": "vless",
      "settings": {
        "clients": [
          {
            "id": "${UUID}",
            "flow": "xtls-rprx-vision"
          }
        ],
        "decryption": "none"
      },
      "streamSettings": {
        "network": "tcp",
        "security": "reality",
        "realitySettings": {
          "show": false,
          "dest": "${REALITY_DEST}",
          "xver": 0,
          "serverNames": [
            "${REALITY_SNI}"
          ],
          "privateKey": "${REALITY_PRIVATE_KEY}",
          "shortIds": [
            "${SHORT_ID}"
          ]
        }
      },
      "sniffing": {
        "enabled": true,
        "destOverride": [
          "http",
          "tls",
          "quic"
        ],
        "routeOnly": true
      }
    }
  ],
  "outbounds": [
    {
      "protocol": "freedom",
      "tag": "direct"
    },
    {
      "protocol": "blackhole",
      "tag": "block"
    }
  ]
}
EOF

    chmod 600 "$XRAY_CONFIG"

    success "配置已生成：$XRAY_CONFIG"
}

test_config() {

    info "检查 Xray 配置..."

    if "$XRAY_BIN" run -test -config "$XRAY_CONFIG"; then
        success "Xray 配置检查通过"
    else
        die "Xray 配置检查失败，请检查：$XRAY_CONFIG"
    fi
}

configure_firewall() {

    echo
    echo "============================================================"
    echo "防火墙"
    echo "============================================================"

    if command -v ufw >/dev/null 2>&1; then

        if ufw status 2>/dev/null | grep -q "Status: active"; then
            info "检测到 UFW，开放 ${XRAY_PORT}/tcp"
            ufw allow "${XRAY_PORT}/tcp" >/dev/null || true
            success "UFW 规则已添加"
        fi

    elif command -v firewall-cmd >/dev/null 2>&1; then

        if firewall-cmd --state >/dev/null 2>&1; then
            info "检测到 firewalld，开放 ${XRAY_PORT}/tcp"

            firewall-cmd \
                --permanent \
                --add-port="${XRAY_PORT}/tcp" >/dev/null || true

            firewall-cmd --reload >/dev/null || true

            success "firewalld 规则已添加"
        fi

    elif command -v iptables >/dev/null 2>&1; then

        info "检测到 iptables"

        if ! iptables -C INPUT \
            -p tcp \
            --dport "$XRAY_PORT" \
            -j ACCEPT >/dev/null 2>&1; then

            iptables -I INPUT \
                -p tcp \
                --dport "$XRAY_PORT" \
                -j ACCEPT || true
        fi

        success "iptables 规则已添加"
    else
        warn "未检测到可自动配置的防火墙"
    fi

    echo
    warn "注意：如果 VPS 服务商还有云防火墙/安全组，请同时开放 TCP ${XRAY_PORT}"
}

restart_xray() {

    info "重新加载 systemd..."

    systemctl daemon-reload

    info "启用 Xray 开机启动..."

    systemctl enable xray >/dev/null 2>&1 || true

    info "重启 Xray..."

    systemctl restart xray

    sleep 2

    if systemctl is-active --quiet xray; then
        success "Xray 已正常运行"
    else

        error "Xray 启动失败"

        echo
        systemctl status xray --no-pager -l || true

        echo
        echo "最近日志："

        journalctl -u xray -n 50 --no-pager || true

        exit 1
    fi
}

generate_vless_link() {

    local encoded_sni

    encoded_sni="$(printf '%s' "$REALITY_SNI" | sed 's/:/%3A/g')"

    VLESS_LINK="vless://${UUID}@${SERVER_IP}:${XRAY_PORT}?encryption=none&flow=xtls-rprx-vision&security=reality&sni=${encoded_sni}&fp=${FINGERPRINT}&pbk=${REALITY_PUBLIC_KEY}&sid=${SHORT_ID}&type=tcp&headerType=none#Xray-Vision-REALITY"
}

save_info() {

    INFO_FILE="${XRAY_DIR}/client-info.txt"

    cat > "$INFO_FILE" <<EOF
============================================================
Xray VLESS + REALITY + Vision
============================================================

Server:
${SERVER_IP}

Port:
${XRAY_PORT}

Protocol:
VLESS

Transport:
TCP

Security:
REALITY

Flow:
xtls-rprx-vision

UUID:
${UUID}

Reality PrivateKey:
${REALITY_PRIVATE_KEY}

Reality PublicKey:
${REALITY_PUBLIC_KEY}

Reality ShortID:
${SHORT_ID}

Reality Destination:
${REALITY_DEST}

Reality SNI:
${REALITY_SNI}

Fingerprint:
${FINGERPRINT}

VLESS Link:
${VLESS_LINK}

============================================================
EOF

    chmod 600 "$INFO_FILE"
}

show_result() {

    echo
    echo
    echo "============================================================"
    echo "              Xray 安装完成"
    echo "============================================================"
    echo
    echo -e "${GREEN}服务器 IP:${NC}"
    echo "  ${SERVER_IP}"
    echo
    echo -e "${GREEN}端口:${NC}"
    echo "  ${XRAY_PORT}"
    echo
    echo -e "${GREEN}协议:${NC}"
    echo "  VLESS"
    echo
    echo -e "${GREEN}传输:${NC}"
    echo "  TCP"
    echo
    echo -e "${GREEN}安全:${NC}"
    echo "  REALITY"
    echo
    echo -e "${GREEN}Flow:${NC}"
    echo "  xtls-rprx-vision"
    echo
    echo -e "${GREEN}UUID:${NC}"
    echo "  ${UUID}"
    echo
    echo -e "${GREEN}Reality PublicKey:${NC}"
    echo "  ${REALITY_PUBLIC_KEY}"
    echo
    echo -e "${GREEN}Reality ShortID:${NC}"
    echo "  ${SHORT_ID}"
    echo
    echo -e "${GREEN}Reality SNI:${NC}"
    echo "  ${REALITY_SNI}"
    echo
    echo -e "${GREEN}Fingerprint:${NC}"
    echo "  ${FINGERPRINT}"
    echo
    echo "============================================================"
    echo "v2rayN 可直接导入以下链接"
    echo "============================================================"
    echo
    echo "${VLESS_LINK}"
    echo
    echo "============================================================"
    echo
    echo "完整配置保存："
    echo "${INFO_FILE}"
    echo
    echo "服务端配置："
    echo "${XRAY_CONFIG}"
    echo
}

show_status() {

    echo
    echo "============================================================"
    echo "Xray 状态"
    echo "============================================================"
    echo

    systemctl status xray --no-pager -l || true

    echo
    echo "监听端口："

    if command -v ss >/dev/null 2>&1; then
        ss -lntp | grep ":${XRAY_PORT}" || true
    elif command -v netstat >/dev/null 2>&1; then
        netstat -lntp | grep ":${XRAY_PORT}" || true
    fi
}

show_config() {

    if [ ! -f "$XRAY_CONFIG" ]; then
        warn "配置文件不存在"
        return
    fi

    echo
    echo "============================================================"
    echo "Xray 配置"
    echo "============================================================"
    echo

    cat "$XRAY_CONFIG"

    echo
}

uninstall_xray() {

    echo
    warn "这会卸载 Xray，但默认保留配置文件。"

    read -rp "确认卸载？输入 YES: " answer

    if [ "$answer" != "YES" ]; then
        echo "已取消"
        return
    fi

    systemctl stop xray 2>/dev/null || true
    systemctl disable xray 2>/dev/null || true

    bash -c "$(curl -L "$INSTALL_SCRIPT_URL")" @ remove

    success "Xray 已卸载"
}

install_menu() {

    clear

    echo "============================================================"
    echo " Xray VLESS + REALITY + XTLS Vision"
    echo "============================================================"
    echo
    echo "1. 安装 / 重装"
    echo "2. 查看状态"
    echo "3. 查看配置"
    echo "4. 重启 Xray"
    echo "5. 查看客户端参数"
    echo "6. 卸载 Xray"
    echo "0. 退出"
    echo

    read -rp "请选择 [0-6]: " MENU

    case "$MENU" in

        1)
            install_main
            ;;

        2)
            show_status
            pause_enter
            install_menu
            ;;

        3)
            show_config
            pause_enter
            install_menu
            ;;

        4)
            systemctl restart xray
            success "Xray 已重启"
            show_status
            pause_enter
            install_menu
            ;;

        5)
            if [ -f "$XRAY_DIR/client-info.txt" ]; then
                cat "$XRAY_DIR/client-info.txt"
            else
                warn "没有找到客户端参数"
            fi

            pause_enter
            install_menu
            ;;

        6)
            uninstall_xray
            pause_enter
            install_menu
            ;;

        0)
            exit 0
            ;;

        *)
            warn "无效选项"
            sleep 1
            install_menu
            ;;
    esac
}

install_main() {

    clear

    echo "============================================================"
    echo " Xray VLESS + REALITY + XTLS Vision 安装"
    echo "============================================================"
    echo

    check_root
    check_os
    check_commands

    install_xray

    echo
    echo "============================================================"
    echo "生成 VLESS + REALITY + Vision 配置"
    echo "============================================================"

    get_server_ip

    echo
    echo "检测到服务器 IP：${SERVER_IP}"

    input_port
    input_reality_dest
    input_server_name
    input_fingerprint

    info "生成 UUID..."
    UUID="$(generate_uuid)"

    if [ -z "$UUID" ]; then
        die "UUID 生成失败"
    fi

    info "生成 Reality X25519 密钥..."
    generate_reality_keys

    info "生成 Reality ShortID..."
    generate_short_id

    create_config

    test_config

    configure_firewall

    restart_xray

    generate_vless_link

    save_info

    show_result

    pause_enter

    install_menu
}

main() {

    check_root

    if [ "$#" -gt 0 ]; then

        case "$1" in

            install)
                install_main
                ;;

            status)
                show_status
                ;;

            config)
                show_config
                ;;

            restart)
                systemctl restart xray
                success "Xray 已重启"
                ;;

            uninstall)
                uninstall_xray
                ;;

            *)
                echo "用法："
                echo
                echo "  $0"
                echo "  $0 install"
                echo "  $0 status"
                echo "  $0 config"
                echo "  $0 restart"
                echo "  $0 uninstall"
                ;;
        esac

    else
        install_menu
    fi
}

main "$@"
