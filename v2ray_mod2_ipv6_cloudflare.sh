...
# IPv6/Cloudflare modification: IPv4优先，IPv4不可用时自动使用IPv6
V6_PROXY=""
IP=$(curl -4 -sL --connect-timeout 8 --max-time 15 https://ip.sb 2>/dev/null)
if [[ $? -ne 0 || -z "$IP" ]]; then
    IP=$(curl -6 -sL --connect-timeout 8 --max-time 15 https://ip.sb 2>/dev/null)
    if [[ $? -ne 0 || -z "$IP" ]]; then
...
getData() {
    if [[ "$TLS" = "true" || "$XTLS" = "true" ]]; then
        ...
        colorEcho ${YELLOW} "  2. 域名需要配置A/AAAA记录"
        colorEcho ${YELLOW} "  3. 如果使用Cloudflare，请开启橙色云朵代理"
        colorEcho ${BLUE} "  4. 如果/root目录下有 v2ray.pem 和 v2ray.key 证书密钥文件，可直接使用"
        echo " "
        ...
        if [[ -f ~/v2ray.pem && -f ~/v2ray.key ]]; then
            ...
            echo ""
            colorEcho ${BLUE} " 正在检查域名DNS..."
            A_RECORDS=$(dig +short A "$DOMAIN" 2>/dev/null)
            AAAA_RECORDS=$(dig +short AAAA "$DOMAIN" 2>/dev/null)
            ALL_RECORDS="${A_RECORDS}
${AAAA_RECORDS}"

            if [[ -z "$(echo "$ALL_RECORDS" | tr -d '[:space:]')" ]]; then
                colorEcho ${RED} " ${DOMAIN} 没有解析记录！"
                colorEcho ${YELLOW} " 请先在DNS中配置 A 或 AAAA 记录。"
                exit 1
            fi

            echo " DNS解析结果："
            [[ -n "$A_RECORDS" ]] && echo " A:" && echo "$A_RECORDS"
            [[ -n "$AAAA_RECORDS" ]] && echo " AAAA:" && echo "$AAAA_RECORDS"

            res=$(echo "$ALL_RECORDS" | grep -F "$IP" || true)
            if [[ -n "$res" ]]; then
                colorEcho ${GREEN} " 域名已直接解析到当前服务器。"
                DNS_MODE="direct"
            else
                echo ""
                colorEcho ${YELLOW} " 注意：域名没有直接解析到当前服务器IP。"
                colorEcho ${YELLOW} " 这通常意味着使用了 Cloudflare/CDN 代理。"
                echo " 当前服务器IP：$IP"
                echo " 域名解析结果："
                echo "$ALL_RECORDS"
                echo ""
                echo " 如果你使用的是 Cloudflare 橙云，请选择 y。"
                echo " 如果没有使用CDN，请选择 n 并检查DNS配置。"
                read -p " 是否使用CDN/Cloudflare代理？[y/n]：" CDN_ANSWER
                if [[ "${CDN_ANSWER,,}" = "y" ]]; then
                    DNS_MODE="cdn"
                    colorEcho ${GREEN} " 已启用CDN/Cloudflare模式。"
                    colorEcho ${GREEN} " 不再要求DNS直接解析到服务器IP。"
                else
                    colorEcho ${RED} " DNS未解析到当前服务器，安装终止。"
                    exit 1
                fi
            fi
        fi
...
outputVlessWS() {
    ...
    echo -e "   ${BLUE}底层安全传输(tls)：${PLAIN}${RED}TLS${PLAIN}"
    if [[ "$DNS_MODE" = "cdn" ]]; then
        echo -e "   ${BLUE}CDN：${PLAIN}${GREEN}Cloudflare/CDN${PLAIN}"
    fi
}
showInfo() {
    res=`status`
